...
const BASE = 'https://reservation.pc.gc.ca'
const CONCURRENCY = 3
const AVAILABILITY_400_COOLDOWN_MS = 5 * 60 * 1000
const AVAILABILITY_400_COOLDOWN_KEY = 'parks_canada_availability_400_cooldown_until'

export class ParksCanadaApiError extends Error {
  constructor(
    public readonly status: number,
    public readonly path: string,
    public readonly body: string,
  ) {
    super(`Parks Canada API error ${status}: ${path}`)
    this.name = 'ParksCanadaApiError'
  }
}

export class ParksCanadaCooldownError extends Error {
  constructor(public readonly cooldownUntil: number) {
    super(`Parks Canada API cooling down until ${new Date(cooldownUntil).toLocaleTimeString()}`)
    this.name = 'ParksCanadaCooldownError'
  }
}
...
function buildFilterData(filters: Filters): string {
  const filterData = []
  // Parks Canada uses this filter for frontcountry reservable campsite searches
  // in the recorded Jasper -> Whistlers flow. We still keep UI filters as local
  // resource checks below because BC Parks' walk-in/double attribute IDs do not
  // map one-to-one here.
  if (filters.noWalkin || filters.noDouble) {
    filterData.push({
...
function splitParksCanadaLocationName(name: string): Pick<Park, 'name' | 'parentName'> {
  const [parent, child] = name.split(/\s+-\s+/, 2).map(part => part.trim())
  if (!parent || !child) return { name }
  return { parentName: parent, name: child }
}
...
export class ParksCanadaProvider {
  ...
  private async api(path: string, params?: Record<string, string>, signal?: AbortSignal): Promise<unknown> {
    ...
    if (!resp.ok) {
      ...
      throw new ParksCanadaApiError(resp.status, path, body)
    }
    return resp.json()
  }
  ...
  private async assertAvailabilityNotCoolingDown(): Promise<void> {
    ...
    if (Date.now() < cooldownUntil) {
      throw new ParksCanadaCooldownError(cooldownUntil)
    }
  }
  ...
  private async getMapAvailability(
    ...
    try {
      ...
      if (!(err instanceof ParksCanadaApiError) || err.status !== 400) throw err
    }
    ...
    try {
      ...
      if (err instanceof ParksCanadaApiError && err.status === 400) {
  ...
  async searchParks(query: string): Promise<Park[]> {
    ...
    return locations
      ...
      .map(loc => {
        const vals = (loc['localizedValues'] as Array<Record<string, string>>)?.[0] ?? {}
        const fullName = vals['shortName'] ?? vals['fullName'] ?? String(loc['resourceLocationId'])
        return {
          id: String(loc['resourceLocationId']),
          ...splitParksCanadaLocationName(fullName),
        }
      })
  }

  private async getResources(campgroundId: string, signal?: AbortSignal): Promise<Record<string, Record<string, unknown>>> {
    const cacheKey = `parks_canada_resources_${campgroundId}`
  ...
  private async getSections(campgroundId: string, signal?: AbortSignal): Promise<Record<string, [string, boolean, string]>> {
    const cacheKey = `parks_canada_sections_${campgroundId}`
    ...
    await Promise.all(candidateMapIds.map(async mapId => {
      ...
      try {
        const mapAvailability = await this.getMapAvailability({
          ...
          peopleCapacityCategoryCounts: JSON.stringify([{ capacityCategoryId: -32767, subCapacityCategoryId: null, count: 1 }]),
          numEquipment: '0',
          filterData,
